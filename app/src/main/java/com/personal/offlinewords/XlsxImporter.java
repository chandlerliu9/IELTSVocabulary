...
final class XlsxImporter {
    static final String[] FIELDS={"order","term","phonetic_ipa","part_of_speech","definition_zh","definition_en","example_en","example_zh","audio_file","status","favorite"};
    ...
    static List<String[]> rows(byte[] data,List<String> shared)throws Exception{List<Map<Integer,String>> raw=new ArrayList<>();XmlPullParser p=parser(data);Map<Integer,String> row=null;int col=-1;String type="";for(int ev=p.getEventType();ev!=XmlPullParser.END_DOCUMENT;ev=p.next()){if(ev==XmlPullParser.START_TAG&&p.getName().equals("row"))row=new HashMap<>();else if(ev==XmlPullParser.START_TAG&&p.getName().equals("c")){col=column(p.getAttributeValue(null,"r"));type=p.getAttributeValue(null,"t");}else if(ev==XmlPullParser.START_TAG&&(p.getName().equals("v")||p.getName().equals("t"))&&row!=null&&col>=0){String v=p.nextText();if("s".equals(type)){int i=Integer.parseInt(v);v=i<shared.size()?shared.get(i):"";}row.put(col,v);}else if(ev==XmlPullParser.END_TAG&&p.getName().equals("row")&&row!=null){raw.add(row);row=null;}}if(raw.isEmpty())throw new IOException("Excel 没有数据");Map<String,Integer> header=new HashMap<>();for(Map.Entry<Integer,String>x:raw.get(0).entrySet())header.put(x.getValue().trim().toLowerCase(Locale.ROOT),x.getKey());if(!header.containsKey("term"))throw new IOException("缺少必需表头 term");if(!header.containsKey("example_en")&&header.containsKey("usage_brief_zh"))header.put("example_en",header.get("usage_brief_zh"));List<String[]> out=new ArrayList<>();for(int r=1;r<raw.size();r++){Map<Integer,String> m=raw.get(r);String[] a=new String[FIELDS.length];for(int i=0;i<a.length;i++){Integer c=header.get(FIELDS[i]);a[i]=c==null?"":m.containsKey(c)?m.get(c):"";}if(!a[1].trim().isEmpty()){if(a[0].isEmpty())a[0]=String.valueOf(out.size()+1);out.add(a);}}return out;}
